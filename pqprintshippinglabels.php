...
class Pqprintshippinglabels extends Module
{
	...
	public function dev()
	{
		...
		#d($this->getMysqlTablesByColumnName('id_carrier'));
	}
	...
	public function __construct()
	{
		...
		$this->hook2 = 'displayBackOfficeTop';
	...
	public static function replaceVars($template, $variables = array())
	{
		...
		// d(self::$replace_vars);
		$template           = preg_replace_callback('/{(?P<tag>[^}]+)}/', array(
			'self',
			'replaceCallback'
		), $template);
	...
	{
		return Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.$this->name.'_pagetypes` WHERE id_pagetype = "'.(int)$id_pagetype.'"');
	}
	...
	*/
	public function generatePDF($preview = true, $data = '')
	{
		...
		$data['orders_ids'] = Tools::substr(@$data['orders_ids'], 0, -1);

		// pt client modificare ca sa ii printeze in functie de comanda -----------------------------------------
		...
		//$orientation = Tools::substr($config['orientation'], 0, 1);
		...
		$template_content1 = $template_content2 = $template_content3 = array();
		...
		foreach ($result as $key => $value) //loop trough orders
		{
			...
			foreach ($labels as $value_product)
			{
				...
				//assign the config
				$this->config = $config;
				$template_content1[$counter] = $template_content;
				$template_content2[$counter] = self::replaceVars($template_content1[$counter], $custom_template_vars);
				$template_content3[$counter] = $this->context->smarty->fetch('string:'.$template_content2[$counter]);	
				$counter++;
			} // end foreach
			...
			foreach ($divs_attrs[$key] as $key2 => $value2)
			{
				...
				if (!empty($patterns[2]))
				{
					...
					if (Tools::substr($divs_attrs2[$key][$key2]['width'], -1) == '%')
					{
						...
						$divs_attrs2[$key][$key2]['width'] = round((Tools::substr($divs_attrs2[$key][$key2]['width'], 0, -1) / 100) * $config['template_width'], 2);
				...
				for ($j = 1; $j <= $config['labels_horizontally']; $j++)
				{
					if (!empty($divs[$cell_count]))
					{
						...
						if (!empty($labels_border))
						{
							...
							$pdf->RoundedRect($x, $y, $config['template_width'], $config['template_height'], $rounded_corners_radius, '1111'); // 2mm in stanga si 2mm in sus
							$pdf->SetLineStyle(array('width' => 0.1, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)));
						}
					}
					$x += $config['template_width'] + $spacing_between_labels_horizontally;
					$cell_count++;
				}
				$y += $config['template_height'] + $spacing_between_labels_vertically;
			}
	...
	public function generateCSV($preview = true, $data='', $sep, $transportista) {
		...
		#$template_content0 = $template_content;

		$template_content1 = $template_content2 = $template_content3 = $bultos_content = '';

		foreach (glob(dirname(__FILE__).'/pdfs/*.txt') as $filename)
		{
			//echo "$filename size ".filesize($filename)."\n";
			@unlink($filename);
		}
		
		$new_filename = 'shipping-labels-'.time().'.txt';

		$file = fopen(dirname(__FILE__).'/pdfs/'.$new_filename, 'w');
		//fwrite($file,"sep=\t".PHP_EOL);
		//fputcsv($file, array_map("utf8_decode",array('Ref. Pedido', 'Nombre', 'Dirección', 'CP', 'Localidad', 'Provincia', 'País', 'Bultos', 'Peso')),"\t");
		$BOM = "\xEF\xBB\xBF"; // UTF-8 BOM
		
		if ($transportista == "redur") {
			fwrite($file,$BOM);
			fputs($file, utf8_encode(implode($sep,array_map("utf8_decode",array('REFERENCIA', 'NOMBRE', 'DIRECCION', 'CODPAIS', 'CP', 'POBLACION', 'PROVINCIA', 'BULTOS', 'KILOS', 'TPORTEE', 'REEMBOLSO', 'OBSERVACIONES', 'REFERENCIA2', 'EMAIL')))."\n"));
		} else if ($transportista == "tipsa") {
			fwrite($file,$BOM);
			fputs($file, utf8_encode(implode($sep,array_map("utf8_decode",array('REF.', 'NOMBRE', 'DIRECCION', 'CP', 'POBLACION', 'PROVINCIA', 'BULTOS', 'KILOS', 'TELEFONO', 'EMAIL', 'OBSERVACIONES')))."\n"));
		} else if ($transportista == "gls") {
				fputs($file, implode($sep,array_map("utf8_decode",array('REF.', 'NOMBRE', 'DIRECCIONM', 'CP', 'POBLACION', 'PROVINCIA', 'BULTOS', 'KILOS', 'TELEFONO', 'EMAIL', 'OBSERVACIONES', 'RETORNO')))."\n");
		} else if ($transportista == "general") {
				fputs($file, implode($sep,array_map("utf8_decode",array('REFERENCIA', 'NOMBRE', 'DIRECCION', 'CODPAIS', 'CP', 'POBLACION', 'PROVINCIA', 'BULTOS', 'KILOS', 'TPORTEE', 'REEMBOLSO', 'OBSERVACIONES', 'REFERENCIA2', 'EMAIL')))."\n");
		}
		...
		foreach ($result as $key => $value)
		{
			...
			$products = $this->getOrderProducts($value['id_order']);
			/*
			$arr_info = array();
			foreach ($products as $key2 => $value2)
			{
				$value2['product_customization'] = Db::getInstance()->executeS('SELECT * FROM  `'._DB_PREFIX_.'customization` c 
				LEFT JOIN '._DB_PREFIX_.'customized_data cd ON (c.id_customization = cd.id_customization) 
				WHERE c.`id_product` = "'.pSQL(@$value2['product_id']).'" 
				AND c.`id_product_attribute` = "'.pSQL(@$value2['product_attribute_id']).'" 
				AND c.`id_cart` = "'.pSQL(@$value2['id_cart']).'"');
				$arr_info[] = $value2;
			}
			//LEFT JOIN '._DB_PREFIX_.'customization_field cf ON (c.id_product = cf.id_product)
			//LEFT JOIN '._DB_PREFIX_.'customization_field_lang cfl ON (cf.id_customization_field = cfl.id_customization_field)
			$products = $arr_info;
			*/
			#d($products);

			//Calculos para el número de bultos, etiquetas y el peso de los distintos productos del envío realizado por Gorka

				$order = new Order((int)$value['id_order']);

				$productos = $order->getProductsDetail();
				$order_weigth = 0;
				$peso_total = 0;
				$numero_bultos = 0;
				$solo_ceros = true;
				...
				foreach ($productos as $productt) {
						//var_dump($product);
						$producto = new Product((int)$productt['id_product']);
						$features = $producto->getFrontFeatures(1);
						$numero_bultos_aux = 0;
						$items_por_bulto = 1;
						$empieza_en_cero = false;

						if ($producto->weight==0) {
							foreach ($features as $feature) {
								if ($feature['name'] == 'Bultos')
									$numero_bultos_aux = (int)$feature['value']*(int)$productt['product_quantity'];
								if ($feature['name'] == 'Peso')
									$peso_total += (int)$feature['value']*(int)$productt['product_quantity'];
								if ($feature['name'] == 'ItemsXBulto')
									$items_por_bulto = ((int)$feature['value'] > 0)?(int)$feature['value']:1;
								if ($feature['name'] == 'EmpiezaEnCero')
									$empieza_en_cero = true;
							}
						} else {
							if ($productt['product_attribute_id']!=null) {
								$attribute_aux = $this->getProductAttributes($productt['product_attribute_id']);
								$peso_aux = $attribute_aux[0]['weight']+$producto->weight;
							} else $peso_aux = $producto->weight;
							$peso_aux = round($peso_aux,2);
							$bultos_aux_int = (int)$peso_aux;
							$peso_total += (int)($bultos_aux_int*$productt['product_quantity']);
							$numero_bultos_aux = round(((100*$peso_aux-100*$bultos_aux_int))*$productt['product_quantity'],0);
						}
						$numero_bultos += $empieza_en_cero?(ceil($numero_bultos_aux/$items_por_bulto)-1):ceil($numero_bultos_aux/$items_por_bulto);
						$solo_ceros = $empieza_en_cero?$solo_ceros:false;
					}
				$order_weigth = $peso_total > 0 ? $peso_total : 1;
				$numero_bultos += $solo_ceros?1:0;
				$numero_bultos = $numero_bultos > 0 ? $numero_bultos : 1;



			$this->context->smarty->assign(array('products' => $products));

			$order_number = ($this->ps_version == '1.4') ? $value['id_order'] : $value['reference'];
			...
			$value['delivery_address']['address1'] = str_replace($remplazos,'',$value['delivery_address']['address1']);
			$value['delivery_address']['address2'] = str_replace($remplazos,'',$value['delivery_address']['address2']); 
			$value['delivery_address']['firstname'] = str_replace($remplazos,'',$value['delivery_address']['firstname']); 
			$value['delivery_address']['lastname'] = str_replace($remplazos,'',$value['delivery_address']['lastname']); 

			switch($value['delivery_address']['id_country']) {
				case "6":
					$estado = "ES";
					break;
				case "8":
			...
			//fputcsv($file, array_map("utf8_decode",array($order_number, $value['delivery_address']['firstname'].' '.$value['delivery_address']['lastname'], $value['delivery_address']['address1'].' '.$value['delivery_address']['address2'], $value['delivery_address']['postcode'], $value['delivery_address']['city'],$value['delivery_address']['state'],$value['delivery_address']['country'], $numero_bultos, $order_weigth)),"\t");
			if ($transportista == "redur"){
				fputs($file, utf8_encode(implode($sep,array_map("utf8_decode",array($order_number, $value['delivery_address']['firstname'].' '.$value['delivery_address']['lastname'], $value['delivery_address']['address1'].' '.$value['delivery_address']['address2'], $estado, $value['delivery_address']['postcode'], $value['delivery_address']['city'], $value['delivery_address']['state'], $numero_bultos, $order_weigth, "P", "0,00", $value['delivery_address']['phone_mobile'], "", $value['email'])))."\n"));
			} else if ($transportista == "tipsa"){
				fputs($file, utf8_encode(implode($sep,array_map("utf8_decode",array($order_number, $value['delivery_address']['firstname'].' '.$value['delivery_address']['lastname'], $value['delivery_address']['address1'].' '.$value['delivery_address']['address2'], $value['delivery_address']['postcode'], $value['delivery_address']['city'], $value['delivery_address']['state'], $numero_bultos, $order_weigth, $value['delivery_address']['phone_mobile'], $value['email'], $value['message'])))."\n"));
			} else if ($transportista == "gls") {
				fputs($file, implode($sep,array_map("utf8_decode",array($order_number, $value['delivery_address']['firstname'].' '.$value['delivery_address']['lastname'], $value['delivery_address']['address1'].' '.$value['delivery_address']['address2'], $value['delivery_address']['postcode'], $value['delivery_address']['city'], $value['delivery_address']['state'], $numero_bultos, $order_weigth, $value['delivery_address']['phone_mobile'], $value['email'], "SUBIDA A PISO CONCERTADA. ", "S")))."\n");
			} else if ($transportista == "general") {
				fputs($file, implode($sep,array_map("utf8_decode",array($order_number, $value['delivery_address']['firstname'].' '.$value['delivery_address']['lastname'], $value['delivery_address']['address1'].' '.$value['delivery_address']['address2'], $estado, $value['delivery_address']['postcode'], $value['delivery_address']['city'], $value['delivery_address']['state'], $numero_bultos, $order_weigth, "P", "0,00", $value['delivery_address']['phone_mobile'], "", $value['email'])))."\n");
			}
		}
	...
	public function getFechadePago($id_order)

	{
		return Db::getInstance()->executeS('
			SELECT date_add FROM `'._DB_PREFIX_.'order_history` oh WHERE (oh.id_order = '.$id_order.' AND (oh.id_order_state = 2 OR oh.id_order_state = 18 OR oh.id_order_state = 37 OR oh.id_order_state = 136) ) LIMIT 1
			');
	}
